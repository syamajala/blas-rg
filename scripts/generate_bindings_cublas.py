...
from template import copyright, cublas_header, footer
import re
    ...
    def to_terra(self):
        ...
        for idx, name in enumerate(self.fortran_sig['c_order']):
            ...
            if name == 'result':
                ...
                body.append(f"var result : {typ}")
                cblas_args.append(f"&result")
                return_result = True
            elif 'dimension' in arg:
                name = name.upper()
        ...
        if self.return_type != "void" and not return_result:
            cblas_call = "return "
        ...
        terra_func = "terra %s_gpu_terra(%s)\n\n%s\nend\n\n" % (self.name, terra_args, body)
        return terra_func
...
def parse_funcs(c_header, sig_file):
    ...
    xml_generator_config = parser.xml_generator_configuration_t(
        ...
        include_paths=['/usr/local/cuda-11.1/include']
    )
...
def generate_gpu_bindings():
    c_header = "/usr/local/cuda-11.1/include/cublas_v2.h"
    sig_file = "fblas.pyf"
