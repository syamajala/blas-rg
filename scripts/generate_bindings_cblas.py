...
from template import copyright, blas_header, cblas_header, task_template, task_template_no_priv, footer, \
    extern_template, extern_template_no_priv
import re
...
class Func():
    ...
    def to_terra(self):
        ...
        for arg in pointer_args:
            self.terra_args.append("pr%s : clib.legion_physical_region_t" % arg)
            self.terra_args.append("fld%s : clib.legion_field_id_t" % arg)
        ...
        terra_func = "terra %s_cpu_terra(%s)\n\n%s\nend\n\n" % (self.name, terra_args, body)
        return terra_func

    def to_regent(self, gpu_func):
        task_args = []
        privileges = []
        body = []
        terra_args = []
        ...
        for arg in self.terra_args:
            ...
            if var is None:
                if typ == 'clib.legion_physical_region_t':
                    n = re.search(r'pr(\w+)', name)
                    n = n.groups()[0]
                    terra_args.append(f"__physical({n})[0]")
                elif typ == 'clib.legion_field_id_t':
        ...
        terra_cpu_call = self.name + "_cpu_terra(%s)"
        terra_cpu_args = ', '.join(terra_args)
        terra_cpu_call = terra_cpu_call % terra_cpu_args

        terra_gpu_call = self.name + "_gpu_terra(%s)"

        layout = ""
        terra_gpu_args = []

        for arg in terra_args:
            if arg == "layout":
                layout = "regentlib.assert(layout == cblas_h.CblasColMajor, 'Expected column major layout.')"
            elif arg.startswith("Trans"):
                terra_gpu_args.append(arg + " - cblas_h.CblasNoTrans")
            elif arg.startswith("Uplo"):
                terra_gpu_args.append(arg + " - cblas_h.CblasUpper")
            elif arg.startswith("Side"):
                terra_gpu_args.append(arg + " - cblas_h.CblasLeft")
            elif arg.startswith("Diag"):
                terra_gpu_args.append(arg + " - cblas_h.CblasNonUnit")
            else:
                terra_gpu_args.append(arg)

        terra_gpu_args = ', '.join(terra_gpu_args)
        terra_gpu_call = terra_gpu_call % terra_gpu_args

        if self.return_type != 'void':
            terra_cpu_call = 'return ' + terra_cpu_call
            terra_gpu_call = 'return ' + terra_gpu_call

        call = f"""var proc = get_executing_processor(__runtime())
    ...
    if clib.legion_processor_kind(proc) == clib.TOC_PROC then
        ...
        if privileges:
            ...
            extern = extern_template % (self.name, task_args, privileges)
        else:
            task = task_template_no_priv % (self.name, task_args, body)
            extern = extern_template_no_priv % (self.name, task_args)
...
def parse_funcs(c_header, sig_file):
    ...
    xml_generator_config = parser.xml_generator_configuration_t(
        ...
        include_paths=['/home/seshu/pkg/include']
    )
...
def generate_cpu_bindings(gpu_funcs={}):
    c_header = "/home/seshu/pkg/include/cblas.h"
    sig_file = "fblas.pyf"
    ...
    for name, func in funcs.items():
        ...
        regent, extern = func.to_regent(gpu_funcs[name])
        regent_funcs.append(regent)
        header_funcs.append(extern)
        bindings.append(func.name)

    with open("cblas.rg", 'w') as f:
        f.write(copyright)
        f.write(cblas_header)
        f.writelines(terra_funcs)
        f.write(footer % ("cblas", "cblas"))

    with open("blas_tasks.rg", "w") as f:
        f.write(copyright)
        f.write(blas_header)
        f.writelines(regent_funcs)
        f.write("\n")
        f.write(footer % ("blas", "blas"))

    with open("blas.rg", "w") as f:
        f.write(copyright)
        f.writelines(header_funcs)
